...
const RegisterRequestDto = z.object({ email: z.string(), password: z.string().min(8) }).passthrough();
const JwtTokenResponseDto = z.object({ jwtToken: z.string() }).partial().passthrough();
const VerificationCodedRequestDto = z.object({ verificationCode: z.string() }).passthrough();
const LoginRequestDto = z.object({ email: z.string(), password: z.string() }).passthrough();
const LoginResponseDto = z
    .object({
        requires2FA: z.boolean(),
        verificationId: z.union([z.null(), z.string()]),
        jwtToken: z.union([z.null(), z.string()]),
    })
    .partial()
    .passthrough();
...
const ChangePasswordRequestDto = z
    .object({ oldPassword: z.string().min(8), newPassword: z.string().min(8) })
    .passthrough();
const EmailRequestDto = z.object({ emailAddress: z.string() }).passthrough();
const VerificationIdResponseDto = z.object({ verificationId: z.string().uuid() }).partial().passthrough();
...
const Error = z.object({ errorMessage: z.string(), errorType: ErrorType.int() }).passthrough();
...
const PagedResultOfEquipmentResponseDto = z
    ...
    .partial()
    .passthrough();
const EquipmentRequestDto = z.object({ name: z.string().max(50), icon: z.string().max(50) }).passthrough();
...
const EventRequestDto = z
    .object({
        ...
        endDate: z.string(),
        isClosed: z.boolean(),
        startTime: z.union([z.null(), z.string()]).optional(),
        endTime: z.union([z.null(), z.string()]).optional(),
    })
    .passthrough();
const PermissionResponseDto = z.object({ id: z.string().uuid(), name: z.string() }).partial().passthrough();
...
const endpoints = makeApi([
    {
        ...
        response: z.object({ verificationId: z.string().uuid() }).partial().passthrough(),
    },
    {
        method: "post",
        path: "/auth/2fa/enable",
        alias: "postAuth2faenable",
        requestFormat: "json",
        parameters: [
            {
                name: "userId",
    ...
    },
    {
        ...
        response: z.object({ jwtToken: z.string() }).partial().passthrough(),
    },
    {
        method: "post",
        path: "/auth/logout",
